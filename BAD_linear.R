...
modelSimsL<-function(iterations = 5, sigma = "none", xTime=25, nSamples = 20, a_1 = 1.1, a_2=0.95){
  ...
  summary_df<-dplyr::select(summ2, contains("looIC"), contains("Estimate"))
   ...
   renameModText<-paste0("fit_L_",sigma,"<-fit_none")
   eval(parse(text=renameModText))
   outputText<-paste0("modelSimsOutput<-list(metrics_df, summary_df, aPlot, fit_L_",sigma,")")
   eval(parse(text=outputText))
   
  return(modelSimsOutput)
}

modelSims_L_homo<-modelSimsL(sigma = "none")
save(modelSims_L_homo, file="modelSims_L_homo.rdata")
2+2

modelSims_L_linear<-modelSimsL(sigma = "linear")
save(modelSims_L_linear, file="modelSims_L_linear.rdata")
2+2

modelSims_L_spline<-modelSimsL(sigma = "spline")
save(modelSims_L_spline, file="modelSims_L_spline.rdata")
2+2

modelSims_L_exp<-modelSimsL(sigma = "exp")
save(modelSims_L_exp, file="modelSims_L_exp.rdata")
2+2

modelSims_L_quad<-modelSimsL(sigma = "quad")
save(modelSims_L_quad, file="modelSimsOutputs/linearOutputs/modelSims_L_quad.rdata")
2+2
...
print(load("modelSimsOutputs/linearSims/modelSims_L_exp.rdata"))

probs <- seq(from=99, to=1, by=-2)/100
avg_pal <- turbo(n=length(probs))
df_test <- rbind(data.frame("treatment"="a",time=1:25,sample="new1"),data.frame("treatment"="b",time=1:25,sample="new2"))
df_pred <- predict(modelSims_L_exp[[4]],df_test,probs=probs) # add fit_spline in place of fit1 for first checks.
test <- cbind(df_test,df_pred)

lExpPlot <- ggplot(test,aes(time,Estimate))+
  facet_wrap(~treatment)+
  lapply(seq(1,49,2),function(i) geom_ribbon(aes_string(ymin=paste("Q",i,sep = ""),ymax=paste("Q",100-i,sep = "")),fill=avg_pal[i],alpha=0.5))+
...
print(load("modelSimsOutputs/linearSims/modelSims_L_homo.rdata"))
...
lHomoPlot <- ggplot(test,aes(time,Estimate))+
  facet_wrap(~treatment)+
  lapply(seq(1,49,2),function(i) geom_ribbon(aes_string(ymin=paste("Q",i,sep = ""),ymax=paste("Q",100-i,sep = "")),fill=avg_pal[i],alpha=0.5))+
  geom_line(data=df,aes(time,y,group=interaction(treatment,sample)),color="gray20", size=0.25)+
  ylab(~~Area~(cm^2))+
