...
priorPL <- prior(lognormal(log(13), .25),nlpar = "a") +
...
df_pred <- predict(fitPL1,df_test,probs=probs) # add fit_spline in place of fit1 for first checks.
...
p <- ggplot(test,aes(time,Estimate))+
  ...
  theme(legend.position='top')
p
ggsave("Fig2/powerLaw_linear_ribbon.png",p, width = 7.04, height=4.04, dpi=300)
