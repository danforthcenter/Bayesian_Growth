...
modelSims<-function(iterations = 10, sigma = "none", xTime=25, nSamples = 20, phi1_a=200, phi2_a=13, phi3_a=3, phi1_b=160, phi2_b=13, phi3_b=3.5){
  ...
  plot<-metrics_df%>%
    dplyr::select(contains("Estimate_phi"))%>%
    pivot_longer(cols = starts_with("Estimate_phi"))%>%
...
splineSumm<-modelSimsOutput_Spline_2[2]
# the spline fit is running into problems making the summary. Need to make that more robust.
...
test<-compareSimModels(modelSimsOutput_None, modelSimsOutput_linear, modelSimsOutput_Spline_2)
...
# Running outside the function to debug
 # # Seems to have worked totally fine. Not sure why the function version is failing somewhere.
iterations = 2
sigma="none"
xTime=25
nSamples = 20
